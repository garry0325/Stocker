...
def generateAllStockPricesWithDate(date):
	...
	for stockItem in response['data9']:
		...
		try: # some stocks may have all zeros
			# name, close, open, high, low, volume, amount, transactions
			prepare = list(stockItem[i] for i in [1, 8, 5, 6, 7, 2, 4, 3])
			...
			prepare[5] = int(prepare[5] / 1000)
			prepare.append('上市')

			info[stockItem[0]] = Stock(prepare[0], prepare[1], prepare[2], prepare[3], prepare[4], offset, prepare[5], prepare[6], prepare[7], prepare[8])

			info[stockItem[0]].date = date
			
		except ValueError:
			info[stockItem[0]] = Stock()
			info[stockItem[0]].date = date


	# for otc stocks
	url = 'https://www.tpex.org.tw/web/stock/aftertrading/otc_quotes_no1430/stk_wn1430_result.php?l=zh-tw&d='
	url = "%s%d/%02d/%02d&se=EW" % (url, date.year-1911, date.month, date.day)
	...
	for stockItem in response['aaData']:
		...
		try:
			prepare = list(stockItem[i] for i in [1, 2, 4, 5, 6, 7, 8, 9])
			offset = float(stockItem[3])
	...
	# getting tse stock performance data
	url = 'https://www.twse.com.tw/exchangeReport/BWIBBU_d?response=json&date='
	url = "%s%d%02d%02d&selectType=ALL" % (url, date.year, date.month, date.day)
	...
	for stockItem in response['data']:
		if(stockItem[2] == '-'):
			info[stockItem[0]].dyield = 0
		else:
			info[stockItem[0]].dyield = float(stockItem[2].replace(',', ''))
		if(stockItem[4] == '-'):
			info[stockItem[0]].peratio = 0
		else:
			info[stockItem[0]].peratio = float(stockItem[4].replace(',', ''))
		if(stockItem[5] == '-'):
			info[stockItem[0]].pbratio = 0
		else:
			info[stockItem[0]].pbratio = float(stockItem[5].replace(',', ''))

	# getting otc stock performance data
	url = 'https://www.tpex.org.tw/web/stock/aftertrading/peratio_analysis/pera_result.php?l=zh-tw&d='
	url = "%s%d/%02d/%02d" % (url, date.year-1911, date.month, date.day)
	...
	for stockItem in response['aaData']:
		if(stockItem[5] == 'N/A'):
			...
			info[stockItem[0]].dyield = float(stockItem[5].replace(',', ''))
		if(stockItem[2] == 'N/A'):
			info[stockItem[0]].peratio = 0
		else:
			info[stockItem[0]].peratio = float(stockItem[2].replace(',', ''))
		if(stockItem[6] == 'N/A'):
			info[stockItem[0]].pbratio = 0
		else:
			info[stockItem[0]].pbratio = float(stockItem[6].replace(',', ''))


	return info

class DateInfo():
	def __init__(self, date1=datetime.datetime(year=2019, month=3, day=5), date2=datetime.datetime.now()):
		
		self.date1 = generateAllStockPricesWithDate(date1)
		time.sleep(2)
		self.date2 = generateAllStockPricesWithDate(date2)

def getStockNameById(id):
	if(id in stockList):
		return stockList[id][0]
	else:
		return None
